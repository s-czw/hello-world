...
**Locked decisions:** **Java 21 / Spring Boot API + Next.js/React web** (D-026 — the API changed from NestJS; web unchanged), PostgreSQL 16 + jOOQ/Flyway, Valkey 8, Traefik TLS, PgBouncer; web↔api contract is OpenAPI-generated · permissive-license-only stack, CI-gated on npm + Maven (JVM runtime is Temurin OpenJDK, GPLv2 + Classpath Exception — documented exception) · **multi-tenant SaaS architecture from day 1** (RLS tenant isolation, S3 storage, per-org quotas, scale-tier roadmap T0→T3 — D-022) · Docker Compose self-hosted at tier T0 · multi-provider auth with per-provider enable/disable — local email/password + Lark sign-in (week 7), Entra ID later · MVP in 6 weeks (20 Jul – 28 Aug 2026) including core work management and the portfolio layer, with the D-022 scope cuts (bar timeline → schedule table; D5, search, templates, stretch ladder deferred to Phase 2) · 4-week pilot from 31 Aug.
