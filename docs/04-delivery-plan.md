...
| R5 | **Roll-up/timeline query cost** — the differentiating read gets slow or is built as N+1 (arch risk #2) | TL | Med / **High** | Single SQL aggregate, denormalized `current_status`, indexes per arch §3.3; **perf-tested against the seeded 50-project/10k-task dataset in Sprint 2, not Sprint 3**; k6 assertion < 500 ms | Sprint-2 measurement > 500 ms → enable the pre-built 60 s Valkey cache flag on the roll-up read and record it; if still red at k6 time, cache TTL rises and a Phase-1 optimization ticket is opened — pilot is not blocked |
...
- **In-pilot failure ladder:** Sev-1 → fix-forward if < 2 h, else roll back to previous tag; data-integrity issue → freeze writes (maintenance page via Traefik), restore from latest verified dump, PD communicates the data-loss window against the RPO.
